...
// ── Destinos dinâmicos (localStorage) ────────────────────────────────────────
...
// ── Mock local (localStorage) — usado quando backend está offline ─────────────
function getDB() {
...
function mockFallback(method, path, body) {
  ...
  // ── itens ──
  if (resource === 'itens') {
    if (method === 'GET' && !id) return Promise.resolve(calcSaldos(db))
    if (method === 'POST') {
      const novo = { ...body, id: db.nextId.itens++, ativo:1 }
      db.itens.push(novo); saveDB(db); return Promise.resolve(novo)
    }
    if (method === 'PUT') {
  ...
  if (resource === 'entradas') {
    if (method === 'GET') {
      ...
      return Promise.resolve(list.map(e => ({
        ...
        item_unidade: db.itens.find(i => i.id===e.item_id)?.unidade || '',
        vlr_total:    e.quantidade * (e.vlr_unit||0)
      })))
    }
    if (method === 'POST') {
      const novo = { ...body, id: db.nextId.entradas++ }
      db.entradas.push(novo); saveDB(db); return Promise.resolve(novo)
    }
    if (method === 'DELETE') {
  ...
  if (resource === 'saidas') {
    ...
    if (method === 'POST') {
      ...
      db.saidas.push(novo); saveDB(db); return Promise.resolve(novo)
    }
    if (method === 'DELETE') {
  ...
  if (resource === 'dashboard') {
    ...
    const entradasVsSaidas = Array.from({ length: diasNoMes }, (_,i) => {
      ...
      const dp  = `${ano}-${mes}-${dia}`
      return {
        dia:      `${dia}/${mes}`,
        entradas: db.entradas.filter(e=>e.data===dp).reduce((s,e)=>s+e.quantidade,0),
        saidas:   db.saidas.filter(s=>s.data===dp).reduce((s,e)=>s+e.quantidade,0),
      }
    ...
    const parado = saldos
      .filter(s => s.saldo>0)
      .map(s => {
        const ultima = db.saidas.filter(x=>x.item_id===s.id).map(x=>x.data).sort().reverse()[0]||null
        const dias   = ultima ? Math.floor((new Date(hoje)-new Date(ultima))/(864e5)) : 9999
        return { ...s, ultimaSaida:ultima, diasParado:dias, valorParado:+(s.saldo*(s.vlr_unit||0)).toFixed(2) }
      })
      .filter(s => s.diasParado>=90)
      .sort((a,b)=>b.diasParado-a.diasParado)

    const movs = [
      ...db.entradas.map(e => ({ tipo:'Entrada', data:e.data, criado_em:e.criado_em||e.data, item_id:e.item_id, qtd:e.quantidade, solicitante:e.fornecedor||e.responsavel||'—' })),
      ...db.saidas.map(s   => ({ tipo:'Saída',   data:s.data, criado_em:s.criado_em||s.data, item_id:s.item_id, qtd:s.quantidade, solicitante:s.solicitante||s.destino||'—' }))
    ].sort((a,b)=>(b.criado_em||'').localeCompare(a.criado_em||'')).slice(0,15)
    ...
    return Promise.resolve({ kpis, alertas:saldos.filter(s=>s.saldo<=5).sort((a,b)=>a.saldo-b.saldo), entradasVsSaidas, curvaABC, composicaoEstoque, parado, movs, porDestino, top10:curvaABC })
  }
